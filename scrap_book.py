...
def scrap_book(url):
    ...
    recent = soup.select_one('li.col-xs-6:nth-child(1)')
    description = soup.select_one('.product_page > p:nth-child(3)')
    return{'upc': upc, 'titre': title, 'incl': price_incl, 'excl': price_excl, 'dispo': availability, 'description': description, 'type': product_type, 'image': image, 'recent': recent}
